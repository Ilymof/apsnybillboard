      ...
      let ads = await Ad.findAll({
        ...
        include: [

          {
            ...
            where: categoryPath ? { path: categoryPath } : {}, // Фильтрация по категории через include
            required: !!categoryPath // Если категория обязательна, INNER JOIN
          },
          {
            model: Subcategory,
            as: 'subcategory',
            attributes: ['subcategoryName'],
            where: subcategoryPath ? { path: subcategoryPath } : null, // Фильтрация по подкатегории
          ...
          },
          {
            ...
            where: regionId ? { id: regionId } : {},  // Фильтрация по региону через include
            required: !!regionId
          },
          {
            model: Photo,
